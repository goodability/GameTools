...
def on_press(key):
    ...
    if key == Key.caps_lock:
        STATUS = STATUS * -1
        if STATUS == 1:
            logging.info("开关打开！", importantText=True)
        else:
            logging.info("开关关闭！", importantText=True)
    # 开关打开才能选择枪械
    if STATUS == 1:
        # print(key),type(key)
        # 识别配件
        ...
        # 重置人物姿态，站立，不屏息
            ...
            logging.info("重置人物姿态")
        # if key!=Key.shift:
        #     pynput.keyboard.HotKey.release(pynput.keyboard.HotKey.parse(key))
...
def on_release(key):
    global isHoldBreath,historyKey
    if key == Key.shift:
        isHoldBreath = False
                ...
                if pressed == True:
                    ...
                    if is_open_mirror == False:
                        ...
                        if holdTime < personConfig.ifNoMirrorShoot:
                            # mirrorImg=globalDetector.getOpenMirrorImg()
                            # is_open_mirror=globalDetector.classIsOpenMirror(mirrorImg)
                            is_open_mirror = True
                            logging.info("开启瞄准镜")
...
def getGunData():
    ...
    logging.info("开枪姿态后座控制权重" + str(round(stepWeight,2)) + "---配件后座控制权重" + str(round(fittingWeights,2)))
